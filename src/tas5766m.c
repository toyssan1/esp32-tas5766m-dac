...
static const char *TAG = "TAS5766";

#define TAS5766M_ASSERT(a, format, b, ...) \
  if ((a) != 0)                            \
  {                                        \
...
esp_err_t tas5766m_read_byte(uint8_t register_name, uint8_t *data)
{
  ...
  i2c_master_write_byte(cmd, TAS5766M_ADDRESS << 1 | WRITE_BIT, ACK_CHECK_EN);
  i2c_master_write_byte(cmd, register_name, ACK_CHECK_EN);
  i2c_master_stop(cmd);
  ret = i2c_master_cmd_begin(I2C_TAS5766M_MASTER_NUM, cmd, 1000 / portTICK_RATE_MS);
  ...
  i2c_master_start(cmd);
  i2c_master_write_byte(cmd, TAS5766M_ADDRESS << 1 | READ_BIT, ACK_CHECK_EN);
                               ...
                               int regLen, uint8_t *data, int datalen)
{
  ...
  ret |= i2c_master_write_byte(cmd, TAS5766M_ADDRESS << 1 | WRITE_BIT, ACK_CHECK_EN);
...
esp_err_t tas5766m_init()
{
  ...
  io_conf.pin_bit_mask = TAS5766M_GPIO_PDN_MASK;
  io_conf.pull_down_en = GPIO_PULLDOWN_DISABLE;
  io_conf.pull_up_en = GPIO_PULLUP_DISABLE;
  ESP_LOGI(TAG, "%s: Running PWR ON sequence on pin: %d", __func__, TAS5766M_GPIO_PDN);
  gpio_config(&io_conf);
  gpio_set_level(TAS5766M_GPIO_PDN, 0);
  vTaskDelay(20 / portTICK_RATE_MS);
  gpio_set_level(TAS5766M_GPIO_PDN, 1);
...
esp_err_t tas5766m_set_volume(uint8_t vol)
{
  ...
  {
    vol = TAS5766M_VOLUME_MIN;
  }
  if (vol > TAS5766M_VOLUME_MAX)
  {
...
esp_err_t tas5766m_set_volume_pct(uint8_t vol)
{
  ...
  if (vol < TAS5766M_VOLUME_PCT_MIN)
    vol = TAS5766M_VOLUME_PCT_MIN;
  if (vol > TAS5766M_VOLUME_PCT_MAX)
    vol = TAS5766M_VOLUME_PCT_MAX;
...
esp_err_t tas5766m_get_dac_mode(TAS5766M_DAC_MODE *mode)
{
    ...
    if (current_value & (1 << 2)) {
        *mode = TAS5766M_DAC_MODE_PBTL;
    } else {
        *mode = TAS5766M_DAC_MODE_BTL;
    }
...
esp_err_t tas5766m_set_dac_mode(TAS5766M_DAC_MODE mode)
{
    ...
    if (mode == TAS5766M_DAC_MODE_PBTL) {
...
esp_err_t tas5766m_set_eq_gain(int band, int gain)
{
  ...
  { 
      ...
      if (reg_value->page != current_page) {                          
          ...
          TAS5766M_SET_BOOK_AND_PAGE(TAS5766M_REG_BOOK_EQ, reg_value->page); 
      }                                                               
...
esp_err_t tas5766m_set_mixer_mode(TAS5766M_MIXER_MODE mode)
{
  ...
  {
  case MIXER_STEREO:
    mixer_l_to_l = TAS5766M_MIXER_VALUE_0DB;
    mixer_r_to_r = TAS5766M_MIXER_VALUE_MUTE;
    mixer_l_to_r = TAS5766M_MIXER_VALUE_MUTE;
    mixer_r_to_l = TAS5766M_MIXER_VALUE_0DB;
    break;

  case MIXER_STEREO_INVERSE:
    mixer_l_to_l = TAS5766M_MIXER_VALUE_MUTE;
    mixer_r_to_r = TAS5766M_MIXER_VALUE_0DB;
    mixer_l_to_r = TAS5766M_MIXER_VALUE_0DB;
    mixer_r_to_l = TAS5766M_MIXER_VALUE_MUTE;
    break;

  case MIXER_MONO:
    mixer_l_to_l = TAS5766M_MIXER_VALUE_MINUS6DB;
    mixer_r_to_r = TAS5766M_MIXER_VALUE_MINUS6DB;
    mixer_l_to_r = TAS5766M_MIXER_VALUE_MINUS6DB;
    mixer_r_to_l = TAS5766M_MIXER_VALUE_MINUS6DB;
    break;
...
esp_err_t tas5766m_get_fs_freq(TAS5766M_FS_FREQ *freq)
{
  ...
  if (ret != ESP_OK)
  {
    ...
    *freq = (TAS5766M_FS_FREQ)reg_value;
  }
  return ret;
}
...
esp_err_t tas5766m_get_power_state(TAS5766M_CTRL_STATE *state)
{
  ...
  if (ret != ESP_OK)
  {
    ...
    *state = (TAS5766M_CTRL_STATE)reg_value;
  }
  return ret;
}
