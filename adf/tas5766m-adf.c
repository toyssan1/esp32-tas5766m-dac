 ...
 */
audio_hal_func_t AUDIO_CODEC_TAS5766M_DEFAULT_HANDLE = {
    .audio_codec_initialize = tas5766m_adf_init,
    .audio_codec_deinitialize = tas5766m_adf_deinit,
    .audio_codec_ctrl = tas5766m_adf_ctrl,
    .audio_codec_config_iface = tas5766m_adf_config_iface,
    .audio_codec_set_mute = tas5766m_adf_set_mute,
    .audio_codec_set_volume = tas5766m_adf_set_volume,
    .audio_codec_get_volume = tas5766m_adf_get_volume,
    .audio_hal_lock = NULL,
    .handle = NULL,
};
...
{
    return tas5766m_set_mute(enable);
}

esp_err_t tas5766m_adf_get_mute(bool *enabled)
{
    return tas5766m_get_mute(enabled);
}
...
{
    // TODO: Implement
    // TAS5766M_HANDLE_t *codec = (TAS5766M_HANDLE_t *)handle;
    // esp_err_t ret = ESP_OK;

    // switch (cmd) {
    //     case AUDIO_CODEC_CTRL_SET_VOLUME: {
    //         uint8_t volume = *(uint8_t *)value;
    //         ret = tas5766m_set_volume(codec, volume);
    //         break;
    //     }
    //     case AUDIO_CODEC_CTRL_START:
    //         ret = tas5766m_power(codec, true);
    //         break;
    //     case AUDIO_CODEC_CTRL_STOP:
    //         ret = tas5766m_power(codec, false);
    //         break;
    //     default:
                                ...
                                audio_hal_codec_i2s_iface_t *iface)
{
    ...
    // return tas5766m_set_i2s_format(codec, cfg->bits, cfg->fmt);
    return ESP_OK;
}