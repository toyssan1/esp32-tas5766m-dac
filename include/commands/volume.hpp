...
private:
    ...
    {
        return -(volume - TAS5766M_VOLUME_DEFAULT) / 2.0;
    }
    ...
    static int volume_command_handler(int argc, char **argv)
    {
        ...
        if (volume < TAS5766M_VOLUME_MIN || volume > TAS5766M_VOLUME_MAX)
        {
            ESP_LOGI(TAG, "Invalid volume level! Must be between %d and %d.", TAS5766M_VOLUME_MIN, TAS5766M_VOLUME_MAX);
            return 1;
        }
